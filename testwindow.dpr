(*
WinMain нужен только в C/C++,в хелпе написано :
The WinMain function is called by the system as the initial entry point
for a Win32-based application. 
int WINAPI WinMain(
...
Т.е. для дельфей он упрятан в недрах модуля System
Hу а пример проги на API вот :
*)
program testwindow;
...
begin
  ...
  begin
    ...
    begin
      MessageBoxA(Handle, 'Password is correct.', 'Password check',
MB_OK);
      FreeMem(Text, TextLength + 1);
      Exit;
    end;
  end;
  MessageBoxA(Handle, 'Password is incorrect.', 'Password check',
MB_OK);
end;
...
BS_PUSHLIKE or BS_TEXT,
  ...
  Label1 := Createwindow('Static', '', WS_VISIBLE or WS_CHILD or
SS_LEFT,
...
WS_VISIBLE or
                          WS_BORDER or ES_PASSWORD, 88, 8, 121, 21,
Handle, 0, Inst, nil);
